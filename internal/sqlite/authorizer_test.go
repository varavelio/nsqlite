...
func TestAuthorizerFunctionWhitelist(t *testing.T) {
	...
	whitelistedScalars := []struct {
		...
		{name: "if", sql: "SELECT IF(1 > 0, 'yes', 'no')"},
		{name: "unixepoch", sql: "SELECT UNIXEPOCH('2024-01-01')"},
		{name: "unistr", sql: "SELECT UNISTR('\\0041')"},
		{name: "unistr_quote", sql: "SELECT UNISTR_QUOTE('A')"},
		{name: "concat_ws", sql: "SELECT CONCAT_WS('-', 'a', 'b', 'c')"},
		{name: "format", sql: "SELECT FORMAT('%s %d', 'val', 42)"},
		{name: "json_array_insert", sql: "SELECT JSON_ARRAY_INSERT('[1,3]', '$[1]', 2)"},
		{name: "json_pretty", sql: "SELECT JSON_PRETTY('{\"a\":1}')"},
		{name: "json_error_position", sql: "SELECT JSON_ERROR_POSITION('invalid')"},
		{name: "json_tree", sql: "SELECT * FROM JSON_TREE('{\"a\":1}')"},
		{name: "jsonb", sql: "SELECT JSONB('{\"a\":1}')"},
		{name: "jsonb_array", sql: "SELECT JSONB_ARRAY(1,2,3)"},
		{name: "jsonb_object", sql: "SELECT JSONB_OBJECT('k','v')"},
		{name: "jsonb_extract", sql: "SELECT JSONB_EXTRACT(JSONB('{\"a\":1}'), '$.a')"},
		{name: "jsonb_insert", sql: "SELECT JSONB_INSERT(JSONB('{}'), '$.a', 1)"},
		{name: "jsonb_set", sql: "SELECT JSONB_SET(JSONB('{}'), '$.a', 1)"},
		{name: "jsonb_replace", sql: "SELECT JSONB_REPLACE(JSONB('{\"a\":1}'), '$.a', 2)"},
		{name: "jsonb_remove", sql: "SELECT JSONB_REMOVE(JSONB('{\"a\":1}'), '$.a')"},
		{name: "jsonb_patch", sql: "SELECT JSONB_PATCH(JSONB('{}'), JSONB('{\"a\":1}'))"},
		{name: "jsonb_array_insert", sql: "SELECT JSONB_ARRAY_INSERT(JSONB('[1,3]'), '$[1]', 2)"},
	}
...
func TestAuthorizerPragmaWhitelist(t *testing.T) {
	...
	allowedPragmas := []struct {
		...
		{name: "user_version_read", sql: "PRAGMA user_version"},
		{name: "schema_version_read", sql: "PRAGMA schema_version"},
		{name: "page_size_read", sql: "PRAGMA page_size"},
		{name: "defer_foreign_keys_read", sql: "PRAGMA defer_foreign_keys"},
	}
	...
	blockedPragmas := []struct {
		...
		{name: "user_version_assign", sql: "PRAGMA user_version = 123"},
		{name: "schema_version_assign", sql: "PRAGMA schema_version = 999"},
		{name: "page_size_assign", sql: "PRAGMA page_size = 8192"},
		{name: "defer_foreign_keys_assign", sql: "PRAGMA defer_foreign_keys = ON"},
		{name: "integrity_check", sql: "PRAGMA integrity_check"},
		{name: "quick_check", sql: "PRAGMA quick_check"},
		{name: "foreign_key_check", sql: "PRAGMA foreign_key_check"},
	}
